...
class FixedMultiFactorStrategy(BaseStrategy):
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        df['market_ma20'] = df.groupby('date')['close'].transform('mean').rolling(20, min_periods=10).mean().shift(1)
        df['market_ma60'] = df.groupby('date')['close'].transform('mean').rolling(60, min_periods=30).mean().shift(1)
        ...
        for date, group in rebalance_days.groupby('date'):
            ...
            for factor in ['pb_inverse', 'profit_factor', 'trend_20d', 'volatility_score']:
                ...
                if std_val > 0:
                    group[f'z_{factor[:4]}'] = (group[factor] - mean_val) / std_val
                else:
                    group[f'z_{factor[:4]}'] = 0
